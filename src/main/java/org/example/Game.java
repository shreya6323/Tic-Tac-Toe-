...
public class Game {
    ...
    private void whoWin(char ch) {
        ...
        if (ch == player.getSymbol()) {
            ...
            println("");
            result += String.format("Hurray \uD83E\uDD73, %s you nailed it \uD83E\uDD29 !: ", player.getName());
            println(result);
            ...
            println("");
            result += "Cyborg win ! Bring on your defense game \uD83D\uDCAA next time !";
            println(result);
   ...
   {
       do {
           String result = String.format("%s choose your playerSymbol from { X, O }: ", player.getName());
       ...
       } while ( !(player.symbol == 'X' || player.symbol == 'O') && println("Sry, Cannot enter anything except 'X' or 'O' \uD83E\uDD25 !"));
 ...
 private  List<Integer> convertPositionIntoRowCol(int position)
    ...
    private void playerPlay()
    {
        ...
        //the last conition of while loop suggests that if the intList does not contains the position choosen by the human then it will again ask him/her
    ...
    {
        String result = String.format("~~~~~~~~~~~~~~~~~~~~~~~~~~~  %s's Move  ~~~~~~~~~~~~~~~~~~~~~~~~~~",name);
        println(result);
        boardobj.showBoard();
    }
    ...
    private void manual() {
        println("\uD83C\uDF69 \uD83C\uDF69 Welcome to the classical Tic-Tac-Toe game \uD83C\uDF69 \uD83C\uDF69");
        println("\uD83D\uDCA1 Start the game by placing Xs and Os alternatively\n" +
    ...
    private void menu() {
        println("\uD83D\uDCA1 If u wanna replay ♻\uFE0F then press R else press aything ( except power button \uD83D\uDE05 )and Enter to quit ⛩\uFE0F !! ");
